    ...
    for f_el, lidx in res.get("fitting_lines", []):
        try:
            fitting_lines_rec[f_el.UniqueId] = sorted(set(
                lines[i][0].UniqueId for i in lidx))
        except Exception:
            pass
    uids = sorted(set(u for lst in line_pipes.values() for u in lst))
    save_lines_record(base, {
        "line_pipes": line_pipes,
        "fitting_lines": fitting_lines_rec,
        "fitting_uids": sorted(fitting_lines_rec.keys()),
