...
.DESCRIPTION
  ...
  2. Downloads the pinned pyRevit installer from the pyrevitlabs/pyRevit
     GitHub releases (version in installer\pyrevit-version.txt, or -PyRevitVersion).
  3. Compiles installer\pyMEP.iss with Inno Setup 6 (ISCC.exe) into ..\dist.

.PARAMETER PyRevitVersion
  pyRevit release to bundle: a version prefix such as 6.5.3 (newest build of
  it) or a full tag such as v6.5.3.26176+2017. Defaults to
  installer\pyrevit-version.txt.
...
if ($PyRevitSetupPath) {
    ...
    # pyRevit release tags carry the full build string (v6.5.3.26176+2017),
    # so the pinned version is matched as a prefix and the newest build of
    # it is taken. Pass the full tag to pin an exact build.
    $headers = @{ "User-Agent" = "pyMEP-installer-build" }
    if ($env:GITHUB_TOKEN) { $headers["Authorization"] = "Bearer $env:GITHUB_TOKEN" }
    $api = "https://api.github.com/repos/pyrevitlabs/pyRevit/releases?per_page=100"
    Write-Host "Looking up $api"
    $rels = Invoke-RestMethod -Uri $api -Headers $headers
    $want = $PyRevitVersion.TrimStart("v", "V")
    $match = $rels | Where-Object {
        -not $_.draft -and (
            $_.tag_name.TrimStart("v", "V") -eq $want -or
            $_.tag_name.TrimStart("v", "V").StartsWith("$want."))
    } | Sort-Object { [datetime]$_.published_at } -Descending | Select-Object -First 1
    if (-not $match) {
        $tags = ($rels | Select-Object -First 15 | ForEach-Object { $_.tag_name }) -join ", "
        throw "No pyRevit release matches '$PyRevitVersion'. Newest tags: $tags"
    }
    Write-Host "pyRevit release: $($match.tag_name)"
    # The per-user pyRevit installer: pyRevit_<build>_signed.exe (not the CLI, not the admin build).
    $asset = $match.assets | Where-Object {
        $_.name -match '^pyRevit_.*signed\.exe$' -and $_.name -notmatch 'CLI' -and $_.name -notmatch 'admin'
    } | Select-Object -First 1
    if (-not $asset) {
        $names = ($match.assets | ForEach-Object { $_.name }) -join ", "
        throw "No per-user pyRevit installer asset in release $($match.tag_name). Assets: $names"
    }
